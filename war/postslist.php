...
<?php if ($posts->id->Visible) { // id ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $posts->SortUrl($posts->id) ?>',1);"><div id="elh_posts_id" class="posts_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $posts->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($posts->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($posts->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php
...
?>
	<?php if ($posts->id->Visible) { // id ?>
		<td<?php echo $posts->id->CellAttributes() ?>><span id="el<?php echo $posts_list->RowCnt ?>_posts_id" class="control-group posts_id">
<span<?php echo $posts->id->ViewAttributes() ?>>
<?php echo $posts->id->ListViewValue() ?></span>
</span><a id="<?php echo $posts_list->PageObjName . "_row_" . $posts_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php
