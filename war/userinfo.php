...
class cuser extends cTable {
	...
	function __construct() {
		...
		// iduser
		$this->iduser = new cField('user', 'user', 'x_iduser', 'iduser', '`iduser`', '`iduser`', 19, -1, FALSE, '`iduser`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->iduser->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['iduser'] = &$this->iduser;

		// login
		$this->_login = new cField('user', 'user', 'x__login', 'login', '`login`', '`login`', 200, -1, FALSE, '`login`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// metadata
		$this->metadata = new cField('user', 'user', 'x_metadata', 'metadata', '`metadata`', '`metadata`', 201, -1, FALSE, '`metadata`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['metadata'] = &$this->metadata;
	...
	function InsertSQL(&$rs) {
		...
		foreach ($rs as $name => $value) {
			if (!isset($this->fields[$name]))
				continue;
			if (EW_ENCRYPTED_PASSWORD && $name == 'metadata')
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('iduser', $rs))
				ew_AddFilter($where, ew_QuotedName('iduser') . '=' . ew_QuotedValue($rs['iduser'], $this->iduser->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@iduser@", ew_AdjustSql($this->iduser->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->metadata->Exportable) $Doc->ExportCaption($this->metadata);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->metadata->Exportable) $Doc->ExportField($this->metadata);
