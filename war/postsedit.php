...
$posts_edit = NULL; // Initialize page object first

class cposts_edit extends cposts {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("postslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// action_name
			$this->action_name->SetDbValueDef($rsnew, $this->action_name->CurrentValue, NULL, $this->action_name->ReadOnly);

			// action_link
			$this->action_link->SetDbValueDef($rsnew, $this->action_link->CurrentValue, NULL, $this->action_link->ReadOnly);
...
// Create page object
if (!isset($posts_edit)) $posts_edit = new cposts_edit();

// Page init
$posts_edit->Page_Init();

// Page main
$posts_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$posts_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var posts_edit = new ew_Page("posts_edit");
posts_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = posts_edit.PageID; // For backward compatibility

// Form object
var fpostsedit = new ew_Form("fpostsedit");

// Validate form
fpostsedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($posts->id->FldErrMsg()) ?>");
...
fpostsedit.Form_CustomValidate = 
...
fpostsedit.ValidateRequired = true;
<?php } else { ?>
fpostsedit.ValidateRequired = false; 
<?php } ?>
...
<?php $posts_edit->ShowPageHeader(); ?>
<?php
$posts_edit->ShowMessage();
?>
<form name="fpostsedit" id="fpostsedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="posts">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_postsedit" class="table table-bordered table-striped">
...
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($posts->id->CurrentValue) ?>">
</span><?php echo $posts->id->CustomMsg ?></td>
	</tr>
<?php } ?>
