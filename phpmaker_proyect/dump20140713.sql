...
CREATE TABLE `fb_posts` (
  ...
  `link` text,
  `name` text,
  `sessionid` varchar(245) DEFAULT '777',
  `description` text,
  `action_name` text,
  `action_link` text,
...
INSERT INTO `jobs` VALUES (0,0,1,1,'2013-01-01 01:01:01');
...
/*!50001 VIEW `getfbjobsinfo` AS select `fb_posts`.`idfb_posts` AS `id`,`fb_posts`.`app_credencial` AS `app_credencial`,`fb_posts`.`target` AS `target`,`fb_posts`.`message` AS `message`,`fb_posts`.`picture` AS `picture`,`fb_posts`.`link` AS `link`,`fb_posts`.`name` AS `name`,`fb_posts`.`description` AS `description`,`fb_posts`.`action_link` AS `action_link`,`fb_posts`.`action_name` AS `action_name`,`fb_posts`.`sessionid` AS `sessionid`,`fb_apps_credenciales`.`app` AS `app`,`fb_apps_credenciales`.`secret` AS `secret` from (`fb_posts` left join `fb_apps_credenciales` on((`fb_apps_credenciales`.`idfb_apps_credenciales` = `fb_posts`.`app_credencial`))) */;
...
-- Dump completed on 2014-07-13 20:40:38
