...
$targets_edit = NULL; // Initialize page object first

class ctargets_edit extends ctargets {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("targetslist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// domainid
			$this->domainid->SetDbValueDef($rsnew, $this->domainid->CurrentValue, NULL, $this->domainid->ReadOnly);

			// fid
			$this->fid->SetDbValueDef($rsnew, $this->fid->CurrentValue, "", $this->fid->ReadOnly);
...
// Create page object
if (!isset($targets_edit)) $targets_edit = new ctargets_edit();

// Page init
$targets_edit->Page_Init();

// Page main
$targets_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$targets_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var targets_edit = new ew_Page("targets_edit");
targets_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = targets_edit.PageID; // For backward compatibility

// Form object
var ftargetsedit = new ew_Form("ftargetsedit");

// Validate form
ftargetsedit.Validate = function() {
...
ftargetsedit.Form_CustomValidate = 
...
ftargetsedit.ValidateRequired = true;
<?php } else { ?>
ftargetsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftargetsedit.Lists["x_domainid"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $targets_edit->ShowPageHeader(); ?>
<?php
$targets_edit->ShowMessage();
?>
<form name="ftargetsedit" id="ftargetsedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="targets">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_targetsedit" class="table table-bordered table-striped">
<?php if ($targets->id->Visible) { // id ?>
	<tr id="r_id"<?php echo $targets->RowAttributes() ?>>
		<td><span id="elh_targets_id"><?php echo $targets->id->FldCaption() ?></span></td>
		<td<?php echo $targets->id->CellAttributes() ?>><span id="el_targets_id" class="control-group">
<span<?php echo $targets->id->ViewAttributes() ?>>
<?php echo $targets->id->EditValue ?></span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($targets->id->CurrentValue) ?>">
</span><?php echo $targets->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
ftargetsedit.Lists["x_domainid"].Options = <?php echo (is_array($targets->domainid->EditValue)) ? ew_ArrayToJson($targets->domainid->EditValue, 1) : "[]" ?>;
</script>
